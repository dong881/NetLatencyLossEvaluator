...
# Random packet data generator for QoS1 and QoS2.
# Resulting string will be in the format "Packet {i} [QoS1]|Packet {i+1} [QoS2]|..."
def generate_packet_data(start: int, end: int, delimiter: str = "|") -> str:

    def generate_qos_label() -> str:
        return random.choice(["QoS1", "QoS2"])
    
    return delimiter.join(f"Packet {i} [{generate_qos_label()}]" for i in range(start, end + 1))

# Based on the delimiter, split the data into QoS1 and QoS2 packets, compress them and return as a dictionary.
def split_and_compress_data(data: str, delimiter: str = "|") -> dict:
    packets = data.split(delimiter)
    qos1_data = delimiter.join(packet for packet in packets if "[QoS1]" in packet)
    qos2_data = delimiter.join(packet for packet in packets if "[QoS2]" in packet)
    
    compressed_qos1 = compress_with_lzma(qos1_data)
    compressed_qos2 = compress_with_lzma(qos2_data)
    
    return {
        "QoS1": compressed_qos1,
        "QoS2": compressed_qos2
    }
    ...
    def send_data(self, runTimes=5):
        ...
        for run in range(runTimes):
            ...
            if run == 0:
                ...
                print(f"Compression ratio QoS1: {len(compressed_data['QoS1']) / len(full_data) * 100:.2f}%")
                print(f"Compression ratio QoS2: {len(compressed_data['QoS2']) / len(full_data) * 100:.2f}%")
                print(f"Total sequences QoS1: {total_sequences_qos1}")
                print(f"Total sequences QoS2: {total_sequences_qos2}")
                print("=====================================")
                self.monitor.record_event('compression_info',
                    original_size=len(full_data),
                    compressed_size=len(compressed_data["QoS1"]), 
                    ratio=len(compressed_data["QoS1"]) / len(full_data) * 100
                )
                self.monitor.record_event('compression_info',
                    original_size=len(full_data),
                    compressed_size=len(compressed_data["QoS2"]), 
                    ratio=len(compressed_data["QoS2"]) / len(full_data) * 100
                )
